#!/usr/bin/env bash
# Build / stage Qwen3-VL NVFP4 bundle (bundles/qwen3_vl_nvfp4/_bundle_build.sh).
#
...
usage() {
  cat <<EOF
Assemble the Qwen3-VL NVFP4 flashcli model bundle (flash_rt/ + tagged *.so under lib/).

Usage:
  bash bundles/qwen3_vl_nvfp4/build.sh [OPTIONS]
...
Note: Qwen3-VL NVFP4 requires SM120. Release: bash release.sh or scripts/release_bundle.sh --bundle qwen3_vl_nvfp4.
EOF
}
...
stage_bundle_runtime() {
  ...
  local has_kernels=0 has_fa2=0 has_vlk=0
  rm -f "${lib_dir}/${kernels_name}" "${lib_dir}/${fa2_name}" "${lib_dir}/${vlk_name}"
  if [[ -f "${cache_dir}/${kernels_name}" ]]; then
    ...
    [[ -f "${cache_dir}/${vlk_name}" ]] && cp -f "${cache_dir}/${vlk_name}" "${lib_dir}/" && has_vlk=1
  fi
  ...
  stage_native_module_to_lib "${build_src}" "${lib_dir}" flash_rt_qwen3_vl_kernels "${vlk_name}" \
    "${PYTHON_MINOR}" && has_vlk=1
  ...
  [[ "${has_vlk}" -eq 1 ]] || die "${vlk_name} missing (build with -DFLASHRT_BUILD_QWEN3_VL=ON)"
  ...
  if [[ "${SM}" != "120" ]]; then
    log "WARNING: Qwen3-VL NVFP4 needs SM120; detected sm=${SM}"
  fi
  ...
  _verify_staged_native_abi "${vlk_name}"
...
maybe_write_tarball() {
  ...
  local name="flashcli-bundle-qwen3_vl_nvfp4-${GIT_REF}-sm${SM}-cu${CUDA_TAG}-${OS_NAME}-${CPU_ARCH}"
...
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  ...
  if [[ "${CUDA_TAG}" == "124" ]]; then
    die "qwen3_vl_nvfp4 (SM120/NVFP4) cannot build cu124: nvcc 12.4 lacks sm_120/sm_120a. Use cu130 only."
  fi
...
log "  flashcli run ${BUNDLE_DIR} --image scene.jpg --prompt 'Describe this image.'"
log "  flashcli serve ${BUNDLE_DIR} --port 8000 --max-pixels 500000"
log "  Release: cd bundles/qwen3_vl_nvfp4 && bash release.sh"
