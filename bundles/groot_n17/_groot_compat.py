"""GROOT bundle-local compat (SM89 .so without fp8_nt_dev). Not part of flashcli core."""
...
def prepare_flash_rt_kernels(*, quiet: bool = False) -> list[str]:
    ...
    try:
        from flash_rt import flash_rt_fa2 as _fa2  # noqa: PLC0415
    except ImportError as exc:
        raise RuntimeError(
            "flash_rt_fa2 is missing from the bundled FlashRT build. "
            "Rebuild: bash bundles/groot_n17/build.sh --repo-root \"$FLASHRT_REPO\""
        ) from exc
    if not hasattr(_fa2, "fwd_fp16"):
        raise RuntimeError(
            "flash_rt_fa2 lacks fwd_fp16. Rebuild FlashRT with FA2 enabled "
            "(see bundles/groot_n17/BUILD.md)."
        )
    ...
    if not hasattr(gemm_cls, _GEMM_FALLBACK):
        raise RuntimeError(
            "flash_rt_kernels.so lacks GROOT FP8 support "
            f"(missing {', '.join(missing)} and {_GEMM_FALLBACK!r}). "
            "Rebuild bundle: bash bundles/groot_n17/build.sh --repo-root /path/to/FlashRT"
        )
    installed = _install_gemm_shims(gemm_cls)
    if not quiet and installed:
        warnings.warn(
            "groot_n17: GemmRunner missing "
            + ", ".join(missing)
