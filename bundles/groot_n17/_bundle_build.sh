#!/usr/bin/env bash
# Build / stage this bundle (bundles/groot_n17/_bundle_build.sh).
#
#   bash build.sh
...
# shellcheck source=../../scripts/lib/bundle_build_python.sh
source "${FLASHCLI_SCRIPTS}/lib/bundle_build_python.sh"
# shellcheck source=../../scripts/lib/cmake_python.sh
source "${FLASHCLI_SCRIPTS}/lib/cmake_python.sh"
...
usage() {
  cat <<EOF
Assemble a GROOT N1.7 flashcli model bundle (flat layout: *.so + flash_rt/ + run.py at bundle root).

Usage:
  bash bundles/groot_n17/build.sh --repo-root DIR [OPTIONS]
...
Options:
  ...
  --python-bin PATH       Override Python for pybind (default: manifest python_abi, auto-install if missing)
  --python-minor NNN      Override python_abi 310/311/312 (default: flashcli-bundle.json)
  --no-install-python     Do not auto-download/install the required Python interpreter
...
stage_vendor_gr00t() {
  local vendor_script="${BUNDLE_DIR}/vendor_gr00t.sh"
  [[ -f "${vendor_script}" ]] || die "Missing ${vendor_script}"
  bash "${vendor_script}"
  local py_bin="${PYTHON_BIN:-python3}"
  if ! "${py_bin}" "${BUNDLE_DIR}/_verify_gr00t_vendor.py" --python "${py_bin}"; then
    die "Vendored gr00t import probe failed (see above)"
  fi
  if ! "${py_bin}" "${BUNDLE_DIR}/_scan_gr00t_inference_imports.py"; then
    die "gr00t inference import scan failed (see above)"
  fi
  if ! "${py_bin}" "${BUNDLE_DIR}/_scan_gr00t_inference_imports.py" --check-manifest; then
    die "flashcli-bundle.json pip is out of sync with gr00t-inference-requirements.txt (run scanner and update manifest)"
  fi
}
...
run_cmake_build() {
  ...
  cmake_append_python3_args cmake_args "${py_bin}"
  ...
  cmake_args+=(-DFA2_HDIMS="64;96;128" -DFA2_DTYPES="fp16;bf16")
...
stage_groot_n17_flash_rt_minimal() {
  local dst="$1"
  ...
  mkdir -p "${dst}/models/groot_n17"
  sync_tree "${src}/models/groot_n17" "${dst}/models/groot_n17"

  for rel in \
    frontends/__init__.py \
    frontends/torch/__init__.py \
    frontends/torch/groot_n17_rtx.py \
    frontends/torch/groot_n17_rtx_fp8.py \
    frontends/torch/groot_n17_rtx_fp16.py \
    frontends/torch/groot_n17_thor.py \
    frontends/torch/groot_n17_thor_fp8.py \
    frontends/torch/groot_n17_thor_fp16.py \
    frontends/torch/_groot_n17_thor_spec.py; do
    _cp_file "${rel}"
  done

  mkdir -p "${dst}/configs"
  _cp_file configs/groot_n17.yaml

  _cp_file hardware/__init__.py
  [[ -f "${src}/hardware/backend.py" ]] && _cp_file hardware/backend.py

  mkdir -p "${dst}/hardware/rtx"
  for rel in attn_backend.py attn_backend_groot_n17.py attn_backend_groot_n17_backbone.py; do
    cp -a "${src}/hardware/rtx/${rel}" "${dst}/hardware/rtx/${rel}"
  done
  cat > "${dst}/hardware/rtx/__init__.py" <<'PY'
"""RTX attention backends (groot_n17 bundle subset)."""
...
PY
  ...
  log "Staged minimal flash_rt/ for groot_n17 ($(find "${dst}" -type f | wc -l) files)"
}
...
finalize_matrix_manifest() {
  ...
  run_manifest_overlay "${BUNDLE_DIR}" "${scan_dir}" "${GEN_MANIFEST}" "${REPO_ROOT}" "${py_bin}" \
    ...
    --has-fp4 "0" \
    --has-fmha "0" \
    --python-minor "${PYTHON_MINOR}" >/dev/null
}
...
stage_bundle_runtime() {
  ...
  if [[ "${skip_py_stage}" -eq 0 ]]; then
    stage_groot_n17_flash_rt_minimal "${py_dir}"
...
maybe_write_tarball() {
  ...
  local name="flashcli-bundle-groot-n17-${GIT_REF}-sm${SM}-cu${CUDA_TAG}-${OS_NAME}-${CPU_ARCH}${py_suffix}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-install-python) AUTO_INSTALL_BUILD_PYTHON=0; shift ;;
    --sm) SM="$2"; shift 2 ;;
...
log "  bash bundles/groot_n17/pack.sh"
