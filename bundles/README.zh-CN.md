# Model Bundles

<p align="right"><a href="README.md">English</a> · <strong>简体中文</strong></p>

本仓库内的参考 bundle **源码**。已发布制品与完整列表见 **[FlashHub](https://flashhub.top)**。终端用户固定 ref（如 `flashcli-bundle/pi05_libero:1.0.4`）— 各 bundle 见 [QUICKSTART](pi05_libero/QUICKSTART.zh-CN.md)。

## 源码 bundle（示例）

| 目录 | Ref | GPU / CUDA |
|------|-----|------------|
| [`pi05_libero/`](pi05_libero/) | `flashcli-bundle/pi05_libero:1.0.4` | **SM89** · cu124/cu130 · **SM120** · cu130 |
| [`groot_n16/`](groot_n16/) | *（本地 dev）* | **SM120** · 仅 cu130 |
| [`qwen_nvfp4/`](qwen_nvfp4/) | `flashcli-bundle/qwen_nvfp4:1.0.1@qwen3`、`@qwen36` | **SM120** · 仅 cu130 |
| [`qwen3_vl_nvfp4/`](qwen3_vl_nvfp4/) | `flashcli-bundle/qwen3_vl_nvfp4:1.0.0` | **SM120** · 仅 cu130 |

Bundle 格式：[bundle_publish_standard.zh-CN.md](../docs/bundle_publish_standard.zh-CN.md) · ref 语法：[model_bundle_standard.zh-CN.md](../docs/model_bundle_standard.zh-CN.md)
