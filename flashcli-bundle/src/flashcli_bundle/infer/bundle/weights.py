...
from flashcli_bundle.manifest_ext import BundleManifest
...
def apply_bundle_env(
    ...
    *,
    ...
    if has_bundle_variants(bundle):
        from flashcli_bundle.variants import resolve_bundle_variant, variant_env
...
def ensure_checkpoint(
    ...
    *,
    ...
    raise RuntimeError(
        "Weight download is only supported on the host flashcli CLI. "
        f"Run: flashcli pull {preset.name}"
    )
...
def validate_weights_spec(bundle: BundleManifest) -> list[str]:
    ...
    errors: list[str] = []
    if has_bundle_variants(bundle):
        from flashcli_bundle.variants import bundle_variants
